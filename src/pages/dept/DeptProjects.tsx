...
import { Search, Plus, Edit, Trash2, FolderOpen, Building2 } from 'lucide-react';
...
export default function DeptProjects() {
  ...
  const [enabledSuppliers, setEnabledSuppliers] = useState<Supplier[]>([]);
  ...
  const fetchEnabledSuppliers = async () => {
    try {
      const { data, error } = await supabase.functions.invoke('dept-api', {
        body: { action: 'get_enabled_suppliers' },
      });
      if (error) throw error;
      setEnabledSuppliers(data.suppliers || []);
    } catch (error) {
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="space-y-2">
              ...
              {enabledSuppliers.length === 0 ? (
                <p className="text-sm text-muted-foreground">暂无启用的供应商，请先在供应商管理中启用</p>
              ) : (
