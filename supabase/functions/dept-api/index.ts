    ...
    switch (action) {
      case "get_dashboard_stats": {
        ...
        const [enabledRes, allSuppliersRes, productsRes] = await Promise.all([
          supabaseAdmin.from("department_suppliers").select("supplier_id, library_type").eq("library_type", "current"),
          supabaseAdmin.from("suppliers").select("id", { count: "exact" }).eq("status", "approved"),
      ...
      case "get_recent_enabled_suppliers": {
        ...
        if (error) throw error;
        return new Response(JSON.stringify(recentEnabled), {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      ...
      case "get_dept_suppliers": {
        ...
        const { data: allSuppliers, error: suppliersError } = await supabaseAdmin
          .from("suppliers")
          .select(
            "id, company_name, supplier_type, contact_name, contact_phone, status, main_products, production_capacity, annual_revenue, employee_count, is_recommended, is_blacklisted, has_objection",
          )
          .eq("status", "approved")
          .order("company_name");
        ...
        const myDeptSupplierIds = new Set(myDeptSuppliers?.map((ds) => ds.supplier_id) || []);
        ...
        const globalEnabledSupplierIds = new Set(allEnabledSuppliers?.map((ds) => ds.supplier_id) || []);
        ...
        switch (libraryTab) {
          ...
          case "backup":
            ...
            filteredSuppliers = filteredSuppliers.filter(
              (s) => s.is_blacklisted !== true && s.has_objection !== true && s.is_recommended !== true,
            );
            break;
          case "blacklist":
            // 拉黑异议库：所有被拉黑或有异议的供应商
            filteredSuppliers = filteredSuppliers.filter((s) => s.is_blacklisted === true || s.has_objection === true);
            break;
          default:
            break;
        }
        ...
        const suppliers = filteredSuppliers.map((s) => {
          ...
          return {
            ...
            contact_name:
              isHiddenByOther && libraryTab !== "organization" ? maskString(s.contact_name) : s.contact_name,
            contact_phone:
              isHiddenByOther && libraryTab !== "organization" ? maskString(s.contact_phone) : s.contact_phone,
      ...
      case "toggle_supplier_hidden": {
        ...
        if (hasNoDepartment) {
          return new Response(JSON.stringify({ error: "该用户未绑定部门" }), {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
        ...
        if (existing) {
          ...
          // 创建新记录（包含隐藏状态）
          const { error } = await supabaseAdmin.from("department_suppliers").insert({
      ...
      case "update_project": {
        ...
        const { error } = await supabaseAdmin.from("projects").update(projectData).eq("id", projectId);
      ...
      case "get_services": {
        ...
        if (error) throw error;
        return new Response(JSON.stringify({ services: services || [] }), {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      ...
      case "import_suppliers": {
        ...
        if (!suppliers || !Array.isArray(suppliers) || suppliers.length === 0) {
          return new Response(JSON.stringify({ error: "无效的供应商数据" }), {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
        ...
        for (let i = 0; i < suppliers.length; i++) {
          ...
          try {
            ...
            const phoneExists = existingAuthUser?.users?.some((u) => u.email === tempEmail || u.phone === contactPhone);
            ...
            const { error: deptSupplierError } = await supabaseAdmin.from("department_suppliers").insert({
              department_id: primaryDeptId,
