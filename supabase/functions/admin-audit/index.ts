...
serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      ...
      // ========== 部门用户审核 ==========
      case 'list_department_registrations': {
        const { status = 'pending' } = params;
        ...
        // 获取部门和用户信息
        const departmentIds = [...new Set(registrations?.map(r => r.department_id) || [])];
        ...
        const registrationsWithDetails = registrations?.map(r => ({
          ...r,
          department: departmentsMap[r.department_id] || null,
          profile: profilesMap[r.user_id] || null,
        })) || [];

        return new Response(
          JSON.stringify({ registrations: registrationsWithDetails }),
          { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }

      case 'approve_department_registration': {
        ...
        if (!existingRole) {
          await supabaseAdmin
            .from('user_roles')
            .insert({ user_id: registration.user_id, role: 'department' });
        }
        ...
        // 更新用户档案
        if (registration.full_name || registration.phone) {
          await supabaseAdmin
            .from('profiles')
            .update({
        ...
        await supabaseAdmin.from('operation_logs').insert({
          user_id: user.id,
          action: 'approve_department_registration',
          target_type: 'department_registrations',
          target_id: registrationId,
          new_data: { status: 'approved', department_id: registration.department_id },
        });
      ...
      case 'reject_department_registration': {
